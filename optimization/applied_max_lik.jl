...
# Step 2: Define the Poisson log-likelihood function
function Poiss_LogLik(λ, y)
    if λ <= 0
        return -Inf  # Log likelihood is undefined for λ <= 0
    end
    return sum(y_i -> y_i * log(λ) - λ, y)  # Ignore ln(y_i!) as it's constant
end

# Plot the log-likelihood
plot(λ -> Poiss_LogLik(λ, y_data), 0.1, 5, lw=3, label="Log Likelihood", xlabel="λ", ylabel="Log Likelihood")
savefig("poisson_log_likelihood_plot.png")  # Save the plot as an image
println("Log likelihood plot saved as 'poisson_log_likelihood_plot.png'.")

#Define the NLopt objective function
function nlopt_objective_fn(params::Vector, grad::Vector, y)
    λ = params[1]  # Extract λ from the parameters
    if λ <= 0
        return -Inf  # Ensure valid domain
    end

    # Calculate the gradient
    if length(grad) > 0
        grad[1] = sum(y_i -> y_i / λ - 1, y)  # Gradient of log-likelihood w.r.t. λ
    end

    # Return the log-likelihood (negated for minimization)
    return -Poiss_LogLik(λ, y)  # Negating to convert to a minimization problem
end

# Set up and run the optimization
opt = Opt(:LD_LBFGS, 1)  # Using gradient-based method for one parameter
lower_bounds!(opt, [1e-6])  # λ > 0 constraint (small positive lower bound)
opt.min_objective = (params, grad) -> nlopt_objective_fn(params, grad, y_data)  # Set the objective function

# Initial guess for λ
initial_guess = [1.0]
objective_value, optimum, result_code = optimize(opt, initial_guess)

# results
println("Optimal λ: ", optimum[1])
println("Maximum log-likelihood: ", -objective_value)  # Negate back for maximization
println("Optimization result code: ", result_code)
