...
# Define the function to compute optimal consumption path
function optimal_consumption_path(a_optim, hh)
    R = hh.R
    a_0 = hh.a_0
    y = hh.y
    
    T = length(y)  # Number of periods (length of income path)
    c = Vector{Float64}(undef, T)  # Consumption path for all periods
    
    # First period consumption (initial assets + income - asset allocation)
    c[1] = R * a_0 + y[1] - a_optim[1]  # First period uses initial assets
    
    # Subsequent periods' consumption based on asset decisions
    for t in 2:T
        c[t] = R * a_optim[t-1] + y[t] - a_optim[t]  # Use a_optim[t-1] for previous assets
    end
    
    return c
end

# Ensure the dimensionality of the optimization is correct (T-1)
T = length(hh.y)  # Number of periods
dim = T - 1  # Dimensionality for asset decisions

# Get the optimal asset allocations (a_optim)
# Assuming that the optimization step has already been performed
# a_optim will have (T-1) elements, representing assets for each period

# Now calculate the optimal consumption path
c_optimal = optimal_consumption_path(a_optim, hh)

# Plot the optimal consumption path
plot(c_optimal, xlabel="Period", ylabel="Consumption", label="Optimal Consumption", lw=3, ylims=(0, 3), title="Optimal Consumption Path")
