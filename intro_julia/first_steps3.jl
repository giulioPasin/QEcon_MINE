...
function multiplication_table(n::Int)
    # Initialize  n x n matrix fille with zeros
    M = zeros(Int, n, n)
    
    # Nested loops for matrix of indeces' product
    for i in 1:n
        for t in 1:n
            M[i, t] = i * t
        end
...
function multiplication_table(n)
    # 1. Initialize the array for values from 1 to n
    values = 1:n
    # 2. Use broadcasting gets multiplication table
